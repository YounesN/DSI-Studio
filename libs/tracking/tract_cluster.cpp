...
{
    std::for_each(clusters.begin(),clusters.end(),boost::checked_delete<Cluster>);
}
struct compare_cluster
{

        bool operator()(Cluster* lhs,Cluster *rhs)
        {
...
void BasicCluster::sort_cluster(void)
{
    ...
    std::vector<Cluster*>::const_iterator from = clusters.begin();
    std::vector<Cluster*>::const_iterator to = clusters.end();
...
void TractCluster::merge_tract(unsigned int tract_index1,unsigned int tract_index2)
{
    ...
    if (cluster_index == 0) // no cluster
    {
        ...
        {
            cluster_index = new Cluster;
            cluster_index->index = clusters.size();
...
const std::vector<unsigned int>* TractCluster::add_connection(unsigned short index,unsigned int track_index)
{
    std::vector<unsigned int>*& voxel_connection_xyz_ptr = voxel_connection[index];

    if (voxel_connection_xyz_ptr == 0)
        voxel_connection_xyz_ptr = new std::vector<unsigned int>();

    voxel_connection_xyz_ptr->push_back(track_index);

    return voxel_connection_xyz_ptr;
}
...
TractCluster::~TractCluster(void)
{
    std::for_each(voxel_connection.begin(),voxel_connection.end(),boost::checked_delete<std::vector<unsigned int> >);
